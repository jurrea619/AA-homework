...
class Cursor
  ...
  def handle_key(key)
    ...
    when :ctrl_c
      ...
      puts "You pushed #{key}. Try again"
      sleep(1)
    end
  end

  # update cursor position with given diff in direction
  def update_pos(diff)
    dx, dy = diff
    new_pos = [@cursor_pos[0] + dx, @cursor_pos[1] + dy]
    # check move is valid before updating position
    @cursor_pos = new_pos if @board.valid_pos?(new_pos)
  end

  # selected boolean used for knowing user turn status
  def toggle_selected
    @selected = !@selected
  end
end